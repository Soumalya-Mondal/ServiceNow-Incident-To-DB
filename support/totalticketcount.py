...
def total_ticket_count(snow_url: str, username: str, password: str, ticket_type: str) -> dict[str, str]:
    ...
    try:
        ...
        log_writer(file_name = 'Total-Ticket-Count', steps = '03', status = 'SUCCESS', message = 'All ServiceNow Parameter Defined')
    except Exception as error:
        log_writer(file_name = 'Total-Ticket-Count', steps = '03', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'message' : f'[Total-Ticket-Count:S03] - {str(error)}', 'ticket_count' : '0'}

    # calling ServiceNow API:S04
    try:
        ...
        if (int(ticket_count_response.status_code) == 200):
            ...
            log_writer(file_name = 'Total-Ticket-Count', steps = '04', status = 'SUCCESS', message = f'Total: "{total_incident_ticket_count}" Incident Tickets Are Present Inside ServiceNow')
            return {'status' : 'SUCCESS', 'message' : 'Total Incident Ticket Count Fetched', 'ticket_count' : str(total_incident_ticket_count)}
        else:
            log_writer(file_name = 'Total-Ticket-Count', steps = '04', status = 'SUCCESS', message = str(ticket_count_response.text))
            return {'status' : 'ERROR', 'message' : str(ticket_count_response.text), 'ticket_count' : '0'}
    except Exception as error:
        log_writer(file_name = 'Total-Ticket-Count', steps = '04', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'message' : f'[Total-Ticket-Count:S04] - {str(error)}'}